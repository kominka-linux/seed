...
fn execute_block(block: &[Stmt], env: &mut Environment, out: &mut dyn Write) -> AppletResult {
    for stmt in block {
        execute_stmt(stmt, env, out)?;
    }
    Ok(())
}

fn execute_stmt(stmt: &Stmt, env: &mut Environment, out: &mut dyn Write) -> AppletResult {
    match stmt {
        Stmt::Print(exprs) => {
            let mut line = String::new();
            if exprs.is_empty() {
                ...
                for (index, expr) in exprs.iter().enumerate() {
                    ...
                    line.push_str(&eval_expr(expr, env)?.as_string());
                }
            }
        ...
        Stmt::If(condition, then_branch) => {
            if eval_expr(condition, env)?.is_truthy() {
                execute_stmt(then_branch, env, out)?;
            }
        }
...
fn eval_expr(expr: &Expr, env: &mut Environment) -> Result<Value, Vec<AppletError>> {
    match expr {
        Expr::Number(value) => Ok(Value::Number(*value)),
        Expr::String(value) => Ok(Value::String(value.clone())),
        Expr::Var(name) => Ok(env.get_var(name)),
        Expr::Field(index) => {
            let field = eval_expr(index, env)?.as_number() as isize;
            Ok(Value::String(env.get_field(field)))
        }
        Expr::UnaryMinus(expr) => Ok(Value::Number(-eval_expr(expr, env)?.as_number())),
        Expr::UnaryPlus(expr) => Ok(Value::Number(eval_expr(expr, env)?.as_number())),
        ...
        Expr::Call(name, args) => match name.as_str() {
            ...
            "or" => {
                ...
                let left = eval_expr(&args[0], env)?.as_number() as u64;
                let right = eval_expr(&args[1], env)?.as_number() as u64;
...
mod tests {
    ...
    fn concatenation_expression_is_parsed() {
        ...
        assert_eq!(eval_expr(&expr, &mut env).expect("value").as_string(), "12");
    }
    ...
    fn field_access_uses_nf_expression() {
        ...
        assert_eq!(eval_expr(&expr, &mut env).expect("value").as_string(), "beta");
    }
}
