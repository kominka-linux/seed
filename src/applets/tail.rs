    ...
    while i < args.len() {
        ...
        match arg.as_str() {
                    ...
                    .unwrap_or(false) =>
            {
                let n: i64 = a[1..]
                    .parse()
                    .map_err(|_| vec![AppletError::new(APPLET, format!("invalid count: '{a}'"))])?;
...
fn tail_lines<R: BufRead, W: Write + ?Sized>(
    reader: &mut R,
    out: &mut W,
    n: i64,
) -> io::Result<()> {
...
fn tail_bytes<R: Read + ?Sized, W: Write + ?Sized>(
    reader: &mut R,
    out: &mut W,
    n: i64,
) -> io::Result<()> {
