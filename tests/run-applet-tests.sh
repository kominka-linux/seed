...
if [ "$is_linux" -eq 1 ]; then
	applets="$applets acpid blkid chroot df dmesg fdisk halt hwclock ifconfig insmod ip killall5 losetup lsmod lsof mdev mknod mount mountpoint netstat ping ping6 pivot_root poweroff reboot rfkill rmmod swapoff swapon switch_root sysctl umount uptime"
fi
